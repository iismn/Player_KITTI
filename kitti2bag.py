...
def save_velo_data(bag, kitti, velo_frame_id, topic):
    ...
    for dt, veloname, labelname in bar(iterable):
        ...
        for t in range(len(labeldata.rgb_id)):
            point = [scan[t][0], scan[t][1], scan[t][2], scan[t][3], scan[t][4], labeldata.rgb_id[t], labeldata.semantic_id[t]]
            scan_rgb.append(point)
        ...
        fields = [PointField('x', 0, PointField.FLOAT32, 1),
                  ...
                  PointField('rgb', 18, PointField.UINT32, 1),
                  PointField('label', 22, PointField.UINT16, 1)]

        pcl_msg = pcl2.create_cloud(header, fields, scan_rgb)
