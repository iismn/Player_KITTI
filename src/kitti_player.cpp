...
int publish_velodyne(ros::Publisher &pub, string infile, std_msgs::Header *header)
{
    ...
    else
    {
        ...
        for (i = 0; input.good() && !input.eof(); i++)
        {
            ...
            double fov_down = -24.8 / 180.0 * 3.141592;
            double fov = (fabs(-24.8) + fabs(2.0)) / 180.0 * 3.141592;

            double proj_y = (pitch + fabs(fov_down)) / fov;
            proj_y *= 64;
            proj_y = floor(proj_y);
            proj_y = min(double(63), proj_y);
            proj_y = max(double(0), proj_y);

            point.ring = proj_y;
            // cout << point.x << " " << point.y << " " << point.z << " " << point.ring << endl;
